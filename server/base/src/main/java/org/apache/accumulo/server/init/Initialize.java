...
public class Initialize {
  ...
  public static boolean initialize(Opts opts, String instanceNamePath, VolumeManager fs) {
    ...
    final Path rootTablet = new Path(fs.choose(Optional.of(RootTable.ID.toString()), configuredTableDirs) + "/" + ServerConstants.TABLE_DIR+ "/" + RootTable.ID + RootTable.ROOT_TABLET_LOCATION);
  ...
  private static void initFileSystem(Opts opts, VolumeManager fs, UUID uuid, Path rootTablet) throws IOException {
    ...
    String tableMetadataTabletDir = fs.choose(Optional.of(MetadataTable.ID.toString()), ServerConstants.getBaseUris()) + "/tables/" + MetadataTable.ID + "/table_info";
    String defaultMetadataTabletDir = fs.choose(Optional.of(MetadataTable.ID.toString()), ServerConstants.getBaseUris()) + "/tables/" + MetadataTable.ID + "/default_tablet";
