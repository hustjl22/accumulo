...
public class MetricsGatherer {
  ...
  private ArrayList<AtomicLongMap<String>> metric;
  ...
  public MetricsGatherer(RFile.Reader reader) {
    iter = reader;
  ...
  public void newLocalityGroup() {
    metric.add(AtomicLongMap.create(new HashMap<String,Long>()));
  ...
  public void addMetric(String myMetric) {
    if (metric.get(numLG - 1).containsKey(myMetric)) {
      metric.get(numLG - 1).getAndIncrement(myMetric);
    } else
      metric.get(numLG - 1).put(myMetric, 1);

    numEntries.set(numLG - 1, numEntries.get(numLG - 1) + 1);

    if (!inBlock.contains(myMetric) && blocks.get(numLG - 1).containsKey(myMetric)) {
      blocks.get(numLG - 1).incrementAndGet(myMetric);
      inBlock.add(myMetric);
    } else if (!inBlock.contains(myMetric) && !blocks.get(numLG - 1).containsKey(myMetric)) {
      blocks.get(numLG - 1).put(myMetric, 1);
      inBlock.add(myMetric);
    }
